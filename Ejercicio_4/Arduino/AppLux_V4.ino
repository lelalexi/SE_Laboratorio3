...
volatile static int modoOp = 0;
volatile static int Brillo=3;
const int numRows = 2;
...
void loop()
{
  ...
  if(modoOp==0){//Modo luminosidad actual (LA)
    Serial.println("LA");
    luminosidadActual();
  }else if(modoOp==1){//Modo luminosidad max y min (LM)
    Serial.println("LM");
    luminosidadMinMax();
  }else if(modoOp==2){//Modo luminosidad promedio (LP)
    Serial.println("LP");
    luminosidadPromedio();
  }else if(modoOp==3){//Ajuste de dimmer (AD)
    Serial.println("AD");
    ajusteDimmer();
  }
}
...
void receiveEvent(int howMany)
{
  ...
  Wire.flush(); //LIMPIO BASURA QUE HAYA QUEDADO EN EL BUFFER
  if(readData){
    parseAndReply();
    Serial.print("Recibido: ");
    Serial.println((char*)recibido);}
...
void parseAndReply()
{
  ...
  {
    case OBTENER_LUX:
      strLux = String((float)(actualLux/100), 2);
    ...
    case OBTENER_MIN:
      minYMax();
      strMin = String((float)(luxVals[indexMin]/100), 2);
    ...
    case OBTENER_MAX:
      minYMax();
      strMax = String((float)(luxVals[indexMax]/100), 2);
...
//FUNCIONES PARA MOSTRAR DATOS LCD -----------------------------------------------------

void luminosidadMinMax(){
  minYMax();
  lcd.setCursor(0,0);
  lcd.print("   Modo LM   ");
  lcd.setCursor(0, 1);
  lcd.print("Min:");
  lcd.setCursor(4, 1);
  lcd.print((float)(luxVals[indexMin]/100),2);
  lcd.setCursor(9, 1);
  lcd.print("Max:");
  lcd.setCursor(13, 1);
  lcd.print((float)(luxVals[indexMax]/100),2);
  delay(10);
}
...
void luminosidadActual(){
  ...
  lcd.print("   Modo LA   ");
  lcd.setCursor(0, 1);
  lcd.print(actualLux,2);
  //delay(50);
}
void luminosidadPromedio(){
  lcd.clear();
